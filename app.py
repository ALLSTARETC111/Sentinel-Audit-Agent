...
# Flask Initialization for Render Health Check
app = Flask(__name__)
...
async def scout_mempool():
    """Real-time monitoring and signal dispatch."""
    try:
        w3 = Web3(Web3.LegacyWebSocketProvider(WSS_URL))
        # Continuous loop for mempool scanning
        async for tx_hash in w3.eth.filter('pending').get_new_entries():
            try:
                ...
                if tx and tx['value'] > w3.to_wei(1, 'ether'):
                    ...
                    signed = Account.sign_message(encode_defunct(text=msg), private_key=SIGNER_KEY)
...
def run_async_loop():
    """Bridge for Python 3.10+ event loop handling."""
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    loop.run_until_complete(scout_mempool())

if __name__ == "__main__":
    # Start the MEV scout in a separate background thread
